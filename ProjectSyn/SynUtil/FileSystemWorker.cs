...
namespace SynUtil
{
    ...
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FileSystemWorker"/> class.
        /// </summary>
        ...
        /// 默认不忽略隐藏文件夹
        /// </summary>
        public bool IgnoreHiddenFolder = false;
        ...
        /// 当文件被忽略时操作
        /// </summary>
        public Action<FileInfo> OnSkipFileActive { get; set; }

        /// <summary>
        /// 当文件夹被忽略时操作
        /// </summary>
        public Action<DirectoryInfo> OnSkipFolderActive { get; set; }
        ...
        private void DoAnalyze(string currentDir)
        {
            ...
            foreach (string fileName in Directory.GetFiles(currentDir))
            {
                ...
                if ((FileMatch != null && !FileMatch(currentFile))
                    || (FileExclude != null && FileExclude(currentFile)))
                {
                    Report("文件：" + fileName + "被跳过");
                    if (OnSkipFileActive != null)
                    {
                        try
                        {
                            OnSkipFileActive(currentFile);
                        }
                        catch { }
                    }
                    continue;
            ...
            foreach (string subDir in Directory.GetDirectories(currentDir))
            {
                ...
                if ((FolderExclude != null && FolderExclude(di)) 
                    || (IgnoreHiddenFolder && (di.Attributes & FileAttributes.Hidden) == FileAttributes.Hidden))
                {
                    Report("文件夹：" + subDir + "被跳过");
                    if (OnSkipFolderActive != null)
                    {
                        try
                        {
                            OnSkipFolderActive(di);
                        }
                        catch { }
                    }
                    continue;
